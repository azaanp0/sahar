...
export const db = {
  ...
  // Products
  getProducts: (): Product[] => load(KEYS.products, DEFAULT_PRODUCTS),
  saveProducts: (products: Product[]) => save(KEYS.products, products),
  addProduct: (p: Product) => {
    const products = load<Product[]>(KEYS.products, DEFAULT_PRODUCTS);
    products.unshift(p);
  ...
  deleteProduct: (id: string) => {
    const products = load<Product[]>(KEYS.products, DEFAULT_PRODUCTS).filter(p => p.id !== id);
    save(KEYS.products, products);
  },

  // Orders
  getOrders: (): Order[] => load(KEYS.orders, DEFAULT_ORDERS),
  updateOrderStatus: (id: string, status: Order['status']) => {
    const orders = load<Order[]>(KEYS.orders, DEFAULT_ORDERS);
