...
const NewArrivalsSection = ({ products }: NewArrivalsSectionProps) => {
    const newProducts = products.filter(p => p.badge === "new" || p.tags?.includes("new")).slice(0, 8);
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-2">
                        <div className="p-2 bg-[#E91E63]/10 dark:bg-[#C2185B]/20 rounded-lg">
                            <Sparkles className="h-5 w-5 text-[#E91E63] dark:text-[#C2185B]" />
                        </div>
                        <div>
                            <h2 className="text-2xl font-bold text-black dark:text-white">وصل حديثاً</h2>
                            <p className="text-sm text-gray-600 dark:text-gray-400">أحدث المنتجات المضافة</p>
                        </div>
                    </div>
